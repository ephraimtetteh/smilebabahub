...
// Every main category must have an entry here.



export type AdMode =
  | "marketplace" // generic buy → BuyModal
  | "food" // order food → OrderModal
  | "apartments" // book property → BookingModal
  | "fashion" // buy fashion → BuyModal (same as marketplace)
  | "delivery" // hire rider → DeliveryRequestModal
  | "pharmacy" // buy pharmacy product → BuyModal with safety note
  | "services"; // request service → contact-based

// ── Shared Ad shape used by all modals ────────────────────────────────────
// Mirrors the serialised Ad returned by GET /ads/:id
export interface AdForModal {
  _id: string;
  title: string;
  description?: string;
  category?: { main?: string; sub?: string };
  price?: { amount?: number; currency?: string; display?: string };
  images?: { url: string; isCover?: boolean; publicId?: string | null }[];
  location?: {
    country?: string;
    region?: string;
    city?: string;
  ...
  postedBy?:
    ...
    | string;
  delivery?: { available?: boolean; fee?: number; option?: string };
  condition?: string;
  negotiable?: string;
  attributes?: { key: string; value: string; label?: string }[];
  boost?: { isBoosted?: boolean; boostTier?: string };
  views?: number;
  saves?: number;
  ...
  expiresAt?: string;
  createdAt: string;
  // Delivery-specific (stored in attributes, surfaced here for convenience)
...
// ── Shared props for all modal components ──────────────────────────────────
export interface ModalProps {
  ad: AdForModal;
  sym: string; // currency symbol: "₵" or "₦"
  onClose: () => void;
}
...
export type Ad = {
  ...
  postedBy:     { _id: string; username: string; profilePicture?: string } | string;
